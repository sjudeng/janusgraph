...
public class JanusGraphGremlinPlugin extends AbstractGremlinPlugin {
    ...
    static {
        ...
        Stream.of(Geo.class.getMethods()).filter(m -> Modifier.isStatic(m.getModifiers())).forEach(METHOD_IMPORTS::add);
        Stream.of(Text.class.getMethods()).filter(m -> Modifier.isStatic(m.getModifiers())).forEach(METHOD_IMPORTS::add);
    }
